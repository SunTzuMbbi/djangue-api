...
export default function PublicInviteLandingScreen() {
  const router = useRouter();
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.topBar}>
        <TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>\u2039 Volver</Text></TouchableOpacity>
        <Text style={styles.title}>Invitación a tanda</Text>
        <View style={{ width: 60 }} />
      </View>
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.body}>Alguien te ha invitado a unirte a una tanda en Djangue. Descarga la app o inicia sesión para unirte.</Text>
      </ScrollView>
      <LegalBar />
    </SafeAreaView>
  );
